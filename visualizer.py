...
def _preload_folder(folder_path):
    ...
    _PRELOADED_TEXTURES[folder_path] = [CoreImage(os.path.join(folder_path, fn)).texture for fn in png_files]
...
class FrameAnimator:
    ...
    def _next_frame(self, dt):
        try: self._rect.texture = next(self._frame_iter)
        except StopIteration: self._frame_iter = cycle(self.textures)
    ...
    def start(self, center_offset=(0, 0)):
        ...
        self._clock_event = Clock.schedule_interval(self._next_frame, 1.0 / float(self.fps))

    def stop(self):
        if self._clock_event: self._clock_event.cancel(); self._clock_event = None
        if self._rect:
            try: self.widget.canvas.after.remove(self._rect)
            except Exception as e: logging.warning(f"FrameAnimator: Failed to remove rect: {e}")
            self._rect = None
...
class HideUIButton(ButtonBehavior, Image):
    def __init__(self, root_layout, **kwargs):
        ...
        self.size_hint = (None, None); self.size = (100, 100)
    def on_press(self): self.source = 'assets/hide_UI_pressed.png'
    def on_release(self):
        self.source = 'assets/hide_UI_normal.png'
        if self.root_layout: self.root_layout.toggle_ui_state()
...
class PanicButton(ButtonBehavior, Image):
    def __init__(self, visualizer, **kwargs):
        ...
        self.size_hint = (None, None); self.size = (100, 100); self.color = (1, 1, 1, 1)
    def on_press(self): self.source = 'assets/red-packet-active.png'; self.visualizer.send_panic()
    def on_release(self): self.source = 'assets/red-packet.png'
...
class KillPacketsButton(ButtonBehavior, Image):
    def __init__(self, visualizer, **kwargs):
        ...
        self.source = 'assets/red-packet.png'
        self.size_hint = (None, None); self.size = (100, 100)
    def on_press(self): self.source = 'assets/red-packet-active.png'; self.visualizer.kill_all_packets()
    def on_release(self): self.source = 'assets/red-packet.png'
...
class PlayPauseButton(ButtonBehavior, Image):
    ...
    def on_press(self): self._long_press_event = Clock.schedule_once(self._do_long_press, 0.5)
    def on_release(self):
        if self._long_press_event:
            self._long_press_event.cancel(); self._long_press_event = None
    ...
    def _do_short_press(self):
        if self.play_state == 'pause':
            self.play_state = 'play'; self.visualizer.is_playing = True
            if self.midi_clock_armed: self.visualizer.send_midi_transport(0xFA)
            self.visualizer.trigger_all_play_nodes()
        else:
            self.play_state = 'pause'; self.visualizer.is_playing = False
            if self.midi_clock_armed: self.visualizer.send_midi_transport(0xFC)
    ...
    def _update_source_image(self):
        ...
        if self.label_widget: self.label_widget.text = "PLAY" if self.play_state == 'pause' else "STOP"
    def pulse(self):
        if not self.midi_clock_armed: return
    ...
    def _set_source_if_unchanged(self, source_to_restore):
        if self.source == 'assets/clock-pulse.png': self.source = source_to_restore

class SelfStrumPopup(FloatLayout):
    def __init__(self, visualizer, circle, is_merged=False, **kwargs):
        super().__init__(**kwargs)
        self.visualizer = visualizer; self.circle = circle; self.is_merged = is_merged
        ...
        with self._content_layout.canvas.before:
            Color(0.5, 0.5, 0.5, 1)
            Line(rectangle=(215, 580, 1550, 450), width=2, joint='miter')
            Color(0, 0, 0, 0.8)
            self.bg_rect = Rectangle(pos=(215, 580), size=(1550, 450))
        from misc.animated_fonts import AnimatedLabel
        label = AnimatedLabel(text='Node Timing and Packet Routing', font_name='assets/Handwrite-01.ttf', font_size='38px',
                              halign='center', valign='middle', size_hint=(1, 0.3), animation_interval=0.125)
        self._content_layout.add_widget(label)

        if self.is_merged:
            self.slider = Slider(min=0, max=100, step=1, background_horizontal='assets/single_slider_bg.png',
                                 border_horizontal=(0, 0, 0, 0), background_width=40, value_track_color=(0, 0, 0, 0),
                                 cursor_image='assets/node_mini_app.png', cursor_size=(100, 100), padding=40,
                                 size_hint=(1, 0.35))
            self.slider.bind(value=self._on_strum_slider_change)
            self.slider.value = self.circle.get('strum_delay_ms', 0)
            self.strum_label = AnimatedLabel(text=f'Strum (ms): {int(self.circle.get("strum_delay_ms", 0))}',
                                             font_name='assets/Handwrite-01.ttf', font_size='34px', color=(1, 1, 1, 1),
                                             halign='right', valign='middle', padding=(0, 60, 0, 0), size_hint=(0.35, 1),
                                             animation_interval=0.125)
            self.strum_row = BoxLayout(orientation='horizontal', size_hint=(1, 0.35), spacing=15)
            self.strum_row.add_widget(self.strum_label); self.strum_row.add_widget(self.slider)
            self._content_layout.add_widget(self.strum_row)

        self.lag_slider = Slider(min=0, max=50, step=1, background_horizontal='assets/single_slider_bg.png',
                                 border_horizontal=(0, 0, 0, 0), background_width=40, value_track_color=(0, 0, 0, 0),
                                 cursor_image='assets/node_mini_app.png', cursor_size=(100, 100), padding=40,
                                 size_hint=(1, 0.35))
        self.lag_slider.bind(value=self._on_lag_slider_change)
        self.lag_slider.value = self.circle.get('lag_ticks', 0)
        self.lag_label = AnimatedLabel(text=f'Lag (ticks): {self.circle.get("lag_ticks", 0)}',
                                       font_name='assets/Handwrite-01.ttf', font_size='34px', color=(1, 1, 1, 1),
                                       halign='right', valign='middle', padding=(0, 105, 0, 0) if not self.is_merged else (0, 60, 0, 0),
                                       size_hint=(0.35, 1), animation_interval=0.125)
        self.lag_row = BoxLayout(orientation='horizontal', size_hint=(1, 0.35), spacing=15)
        self.lag_row.add_widget(self.lag_label); self.lag_row.add_widget(self.lag_slider)
        ...
        if self.circle.get('connection_mode', 0) == 1:
            ...
            if max_conns > 0:
                ...
                self.conn_slider = Slider(min=0, max=max_conns, step=1, background_horizontal='assets/single_slider_bg.png',
                                          border_horizontal=(0, 0, 0, 0), background_width=40, value_track_color=(0, 0, 0, 0),
                                          cursor_image='assets/node_mini_app.png', cursor_size=(100, 100), padding=40,
                                          size_hint=(1, 0.35))
                self.conn_slider.bind(value=self._on_conn_slider_change)
                self.conn_slider.value = init_val
                self.conn_label = AnimatedLabel(text=f'Conn (1-{max_conns}): {init_val if init_val > 0 else "R"}',
                                                font_name='assets/Handwrite-01.ttf', font_size='34px', color=(1, 1, 1, 1),
                                                halign='right', valign='middle', padding=(0, 60, 0, 0), size_hint=(0.35, 1),
                                                animation_interval=0.125)
                self.conn_row = BoxLayout(orientation='horizontal', size_hint=(1, 0.35), spacing=15)
                self.conn_row.add_widget(self.conn_label); self.conn_row.add_widget(self.conn_slider)
    ...
    def _update_size(self, instance, value):
        ...
        if hasattr(self, 'lag_label'): self.lag_label.text = f'Lag (ticks): {self.circle.get("lag_ticks", 0)}'
        if hasattr(self, 'strum_label'): self.strum_label.text = f'Strum (ms): {int(self.slider.value) if self.is_merged else 0}'
    ...
    def _on_strum_slider_change(self, instance, value):
        ...
        if hasattr(self, 'strum_label'): self.strum_label.text = f'Strum (ms): {int(value)}'

    def _on_lag_slider_change(self, instance, value):
        self.circle['lag_ticks'] = int(value)
        if hasattr(self, 'lag_label'): self.lag_label.text = f'Lag (ticks): {int(value)}'
    ...
    def on_touch_down(self, touch):
        if self._content_layout.collide_point(*touch.pos): return super().on_touch_down(touch)
...
class ResetButton(ButtonBehavior, Image):
    ...
    def __init__(self, **kwargs):
        ...
        self._long_press_event = None; self.long_press_time = 1.0
    ...
    def _load_state_textures(self, state):
        ...
        if not os.path.exists(folder): logging.warning(f"ResetButton: Frame folder not found: {folder}"); return []
        png_files = sorted(f for f in os.listdir(folder) if f.lower().endswith('.png'))
        if not png_files: logging.warning(f"ResetButton: No PNG frames found in {folder}"); return []
        return [CoreImage(os.path.join(folder, fn)).texture for fn in png_files]

    def _load_all_textures(self):
        return {state: self._load_state_textures(state) for state in self._frames_dir}

    def set_state(self, new_state: str, force_reload=False):
        if self.state_name == new_state and hasattr(self, 'texture') and not force_reload: return
    ...
    def _next_frame(self, dt):
        try:
            if self._frame_cycle: self.texture = next(self._frame_cycle)
        except Exception as e:
            logging.warning(f"ResetButton: Failed to get next frame: {e}")
            if self._anim_event: self._anim_event.cancel(); self._anim_event = None

    def _on_press(self, *args): self._long_press_event = Clock.schedule_once(self._do_long_press, self.long_press_time)
    ...
    def _do_short_press(self):
        if self.state_name == self.STATE_ARMED:
            ...
            if hasattr(self, "visualizer"): self.visualizer.reset_aggressive(callback=self._reset_done)
            else: self._reset_done()
        elif self.state_name == self.STATE_DISARMED:
            logging.info("ResetButton: Short press. Opening Goodies Menu.")
            app = App.get_running_app()
            if app: app.switch_to_widget('goodies_menu')
    def _do_long_press(self, dt):
        self._long_press_event = None
        if self.state_name == self.STATE_DISARMED:
            logging.info("ResetButton: Long press. Button ARMED.")
            self.set_state(self.STATE_ARMED)
    def _reset_done(self, *_, **__): self.set_state(self.STATE_DISARMED)
    def on_touch_down(self, touch):
        if (self.state_name == self.STATE_ARMED and self.parent and not self.collide_point(*touch.pos)):
            self.set_state(self.STATE_DISARMED); return False
        return super().on_touch_down(touch)
    def on_parent(self, widget, parent):
        if parent is None and self._anim_event: self._anim_event.cancel(); self._anim_event = None
